...
class window.CharaMake
  ...
  Run: do ->
    ...
    inputblock.style.backgroundColor="rgb(25,50,25)"
    inputblock.style.position="absolute"
    inputblock.style.left=(document.body.clientWidth/2)-128+"px"
    inputblock.style.top=(document.body.clientHeight/2)-24+"px"
    inputblock.style.width=256+"px"
    inputblock.style.height=32+"px"
    inputblock.style.paddingTop=12+"px"
    inputblock.style.textAlign="center"
    inputblock.style.fontSize=16+"px"
    inputblock.style.zIndex=4
    inputblock.style.visibility="hidden"
    input=document.createElement("input")
    input.id="input"
    inputblock.appendChild(input)
    document.body.appendChild(inputblock)
    inputblock.elm=document.getElementById("inputblock")
    inputblock.input=document.getElementById("input")
    ...
    return (Process_Seed,Renderer_Order,Input) ->
      ...
      else if (flag&0b00010000) is 0b00010000
        Renderer_Order.push(Renderer.DrawBg)
        Renderer_Order.push(Renderer.Profile(player,"最後に、君の名前を教えてくれ。"))

        if(document.activeElement is inputblock.input)
          if (Input.state.Key[13])
            Process_Seed.Player=player
            Process_Seed.Player.name=inputblock.input.value
            document.body.removeChild(inputblock)
            Process_Seed.Player.flag.CONTROL_STATE=Process_Seed.Player.flag.CONTROL_STATE|0b0001
            Process_Seed.Player.flag.Menu=0b00000001
            Process_Seed.Player.item.push(new Potion())
            Process_Seed.Player.item.push(new Potion())
            Process_Seed.Player.item.push(new Potion())
